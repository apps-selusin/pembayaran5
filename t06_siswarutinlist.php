...
$t06_siswarutin_list = NULL; // Initialize page object first

class ct06_siswarutin_list extends ct06_siswarutin {
	...
	// Page object name
	var $PageObjName = 't06_siswarutin_list';

	// Grid form hidden field names
	var $FormName = 'ft06_siswarutinlist';
	...
	function __construct() {
		...
		if (!isset($GLOBALS["t06_siswarutin"]) || get_class($GLOBALS["t06_siswarutin"]) == "ct06_siswarutin") {
			$GLOBALS["t06_siswarutin"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["t06_siswarutin"];
		}
		...
		$this->AddUrl = "t06_siswarutinadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "t06_siswarutindelete.php";
		$this->MultiUpdateUrl = "t06_siswarutinupdate.php";
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 't06_siswarutin', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption ft06_siswarutinlistsrch";
	...
	function Page_Terminate($url = "") {
		...
		// Export
		global $EW_EXPORT, $t06_siswarutin;
		if ($this->CustomExport <> "" && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, $EW_EXPORT)) {
				$sContent = ob_get_contents();
			if ($gsExportFile == "") $gsExportFile = $this->TableVar;
			$class = $EW_EXPORT[$this->CustomExport];
			if (class_exists($class)) {
				$doc = new $class($t06_siswarutin);
				$doc->Text = $sContent;
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"ft06_siswarutinlistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = FALSE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"ft06_siswarutinlistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = FALSE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.ft06_siswarutinlist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
...
// Create page object
if (!isset($t06_siswarutin_list)) $t06_siswarutin_list = new ct06_siswarutin_list();

// Page init
$t06_siswarutin_list->Page_Init();

// Page main
$t06_siswarutin_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t06_siswarutin_list->Page_Render();
?>
...
ft06_siswarutinlist.ValidateRequired = true;
<?php } else { ?>
ft06_siswarutinlist.ValidateRequired = false; 
<?php } ?>
...
<?php if ($t06_siswarutin_list->TotalRecs > 0 && $t06_siswarutin_list->ExportOptions->Visible()) { ?>
<?php $t06_siswarutin_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php echo $Language->SelectionForm(); ?>
<div class="clearfix"></div>
</div>
<?php if (($t06_siswarutin->Export == "") || (EW_EXPORT_MASTER_RECORD && $t06_siswarutin->Export == "print")) { ?>
<?php
if ($t06_siswarutin_list->DbMasterFilter <> "" && $t06_siswarutin->getCurrentMasterTable() == "t04_siswa") {
	if ($t06_siswarutin_list->MasterRecordExists) {
?>
...
<?php
	...
	if ($bSelectLimit) {
		if ($t06_siswarutin_list->TotalRecs <= 0)
			$t06_siswarutin_list->TotalRecs = $t06_siswarutin->SelectRecordCount();
	} else {
		if (!$t06_siswarutin_list->Recordset && ($t06_siswarutin_list->Recordset = $t06_siswarutin_list->LoadRecordset()))
			$t06_siswarutin_list->TotalRecs = $t06_siswarutin_list->Recordset->RecordCount();
	}
	$t06_siswarutin_list->StartRec = 1;
	if ($t06_siswarutin_list->DisplayRecs <= 0 || ($t06_siswarutin->Export <> "" && $t06_siswarutin->ExportAll)) // Display all records
		$t06_siswarutin_list->DisplayRecs = $t06_siswarutin_list->TotalRecs;
	if (!($t06_siswarutin->Export <> "" && $t06_siswarutin->ExportAll))
		$t06_siswarutin_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$t06_siswarutin_list->Recordset = $t06_siswarutin_list->LoadRecordset($t06_siswarutin_list->StartRec-1, $t06_siswarutin_list->DisplayRecs);

	// Set no record found message
	if ($t06_siswarutin->CurrentAction == "" && $t06_siswarutin_list->TotalRecs == 0) {
		if ($t06_siswarutin_list->SearchWhere == "0=101")
			$t06_siswarutin_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
...
<?php
$t06_siswarutin_list->ShowMessage();
?>
<?php if ($t06_siswarutin_list->TotalRecs > 0 || $t06_siswarutin->CurrentAction <> "") { ?>
<div class="panel panel-default ewGrid t06_siswarutin">
<form name="ft06_siswarutinlist" id="ft06_siswarutinlist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t06_siswarutin_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t06_siswarutin_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="t06_siswarutin">
<?php if ($t06_siswarutin->getCurrentMasterTable() == "t04_siswa" && $t06_siswarutin->CurrentAction <> "") { ?>
<input type="hidden" name="<?php echo EW_TABLE_SHOW_MASTER ?>" value="t04_siswa">
<input type="hidden" name="fk_id" value="<?php echo $t06_siswarutin->siswa_id->getSessionValue() ?>">
<?php } ?>
<div id="gmp_t06_siswarutin" class="<?php if (ew_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php if ($t06_siswarutin_list->TotalRecs > 0 || $t06_siswarutin->CurrentAction == "gridedit") { ?>
<table id="tbl_t06_siswarutinlist" class="table ewTable">
...
// Header row
$t06_siswarutin_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$t06_siswarutin_list->RenderListOptions();

// Render list options (header, left)
$t06_siswarutin_list->ListOptions->Render("header", "left");
?>
<?php if ($t06_siswarutin->siswa_id->Visible) { // siswa_id ?>
	...
	<?php } else { ?>
		<th data-name="siswa_id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t06_siswarutin->SortUrl($t06_siswarutin->siswa_id) ?>',2);"><div id="elh_t06_siswarutin_siswa_id" class="t06_siswarutin_siswa_id">
...
<?php if ($t06_siswarutin->rutin_id->Visible) { // rutin_id ?>
	...
	<?php } else { ?>
		<th data-name="rutin_id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t06_siswarutin->SortUrl($t06_siswarutin->rutin_id) ?>',2);"><div id="elh_t06_siswarutin_rutin_id" class="t06_siswarutin_rutin_id">
...
<?php if ($t06_siswarutin->Nilai->Visible) { // Nilai ?>
	...
	<?php } else { ?>
		<th data-name="Nilai"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t06_siswarutin->SortUrl($t06_siswarutin->Nilai) ?>',2);"><div id="elh_t06_siswarutin_Nilai" class="t06_siswarutin_Nilai">
...
$t06_siswarutin_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($t06_siswarutin->ExportAll && $t06_siswarutin->Export <> "") {
	$t06_siswarutin_list->StopRec = $t06_siswarutin_list->TotalRecs;
} else {

	// Set the last record to display
	if ($t06_siswarutin_list->TotalRecs > $t06_siswarutin_list->StartRec + $t06_siswarutin_list->DisplayRecs - 1)
		$t06_siswarutin_list->StopRec = $t06_siswarutin_list->StartRec + $t06_siswarutin_list->DisplayRecs - 1;
	else
		$t06_siswarutin_list->StopRec = $t06_siswarutin_list->TotalRecs;
}
...
if ($t06_siswarutin_list->Recordset && !$t06_siswarutin_list->Recordset->EOF) {
	...
	$bSelectLimit = $t06_siswarutin_list->UseSelectLimit;
	if (!$bSelectLimit && $t06_siswarutin_list->StartRec > 1)
		$t06_siswarutin_list->Recordset->Move($t06_siswarutin_list->StartRec - 1);
} elseif (!$t06_siswarutin->AllowAddDeleteRow && $t06_siswarutin_list->StopRec == 0) {
	$t06_siswarutin_list->StopRec = $t06_siswarutin->GridAddRowCount;
}
...
$t06_siswarutin_list->ListOptions->Render("body", "left", $t06_siswarutin_list->RowCnt);
?>
	<?php if ($t06_siswarutin->siswa_id->Visible) { // siswa_id ?>
...
// Close recordset
if ($t06_siswarutin_list->Recordset)
	$t06_siswarutin_list->Recordset->Close();
?>
<div class="panel-footer ewGridLowerPanel">
<?php if ($t06_siswarutin->CurrentAction <> "gridadd" && $t06_siswarutin->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($t06_siswarutin_list->Pager)) $t06_siswarutin_list->Pager = new cPrevNextPager($t06_siswarutin_list->StartRec, $t06_siswarutin_list->DisplayRecs, $t06_siswarutin_list->TotalRecs) ?>
<?php if ($t06_siswarutin_list->Pager->RecordCount > 0 && $t06_siswarutin_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($t06_siswarutin_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $t06_siswarutin_list->PageUrl() ?>start=<?php echo $t06_siswarutin_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($t06_siswarutin_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $t06_siswarutin_list->PageUrl() ?>start=<?php echo $t06_siswarutin_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $t06_siswarutin_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($t06_siswarutin_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $t06_siswarutin_list->PageUrl() ?>start=<?php echo $t06_siswarutin_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($t06_siswarutin_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $t06_siswarutin_list->PageUrl() ?>start=<?php echo $t06_siswarutin_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $t06_siswarutin_list->Pager->PageCount ?></span>
</div>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $t06_siswarutin_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $t06_siswarutin_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $t06_siswarutin_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
</form>
<?php } ?>
...
<?php
	foreach ($t06_siswarutin_list->OtherOptions as &$option)
...
<?php } ?>
<?php if ($t06_siswarutin_list->TotalRecs == 0 && $t06_siswarutin->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($t06_siswarutin_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php } ?>
<script type="text/javascript">
ft06_siswarutinlist.Init();
</script>
<?php
$t06_siswarutin_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t06_siswarutin_list->Page_Terminate();
?>
