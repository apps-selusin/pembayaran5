...
$v07_siswarutinbayar_grid = NULL; // Initialize page object first

class cv07_siswarutinbayar_grid extends cv07_siswarutinbayar {
	...
	// Page object name
	var $PageObjName = 'v07_siswarutinbayar_grid';

	// Grid form hidden field names
	var $FormName = 'fv07_siswarutinbayargrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["v07_siswarutinbayar"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function ClearInlineMode() {
		$this->a_Nilai->FormValue = ""; // Clear form value
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->a_id->CurrentValue;
					if ($sKey <> "") $sKey .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
					$sKey .= $this->b_id->CurrentValue;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->a_id->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->b_id->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		if (!$this->a_Nilai->FldIsDetailKey) {
	...
	function RestoreFormValues() {
		...
		if ($this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
			$this->a_id->CurrentValue = $this->a_id->FormValue;
		if ($this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				$this->a_id->CurrentValue = strval($arKeys[0]); // a_id
			else
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->b_id->CurrentValue = strval($arKeys[1]); // b_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		$this->Per_Thn_Bln_Byr_Text->ViewCustomAttributes = "";
			...
			$this->a_Nilai->EditValue = ew_HtmlEncode($this->a_Nilai->CurrentValue);
			$this->a_Nilai->PlaceHolder = ew_RemoveHtml($this->a_Nilai->FldCaption());
			if (strval($this->a_Nilai->EditValue) <> "" && is_numeric($this->a_Nilai->EditValue)) {
			$this->a_Nilai->EditValue = ew_FormatNumber($this->a_Nilai->EditValue, -2, -2, -2, -2);
	...
	function ValidateForm() {
		...
		if (!$this->a_Nilai->FldIsDetailKey && !is_null($this->a_Nilai->FormValue) && $this->a_Nilai->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->a_Nilai->FldCaption(), $this->a_Nilai->ReqErrMsg));
		}
		if (!ew_CheckNumber($this->a_Nilai->FormValue)) {
			ew_AddMessage($gsFormError, $this->a_Nilai->FldErrMsg());
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "v06_siswa") {
			...
			if ($GLOBALS["v06_siswa"]->EventCancelled) $this->EventCancelled = TRUE;
		}
