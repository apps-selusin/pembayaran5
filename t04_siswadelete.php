...
$t04_siswa_delete = NULL; // Initialize page object first

class ct04_siswa_delete extends ct04_siswa {
	...
	// Page object name
	var $PageObjName = 't04_siswa_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("t04_siswalist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in t04_siswa class, t04_siswainfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("t04_siswalist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($t04_siswa_delete)) $t04_siswa_delete = new ct04_siswa_delete();

// Page init
$t04_siswa_delete->Page_Init();

// Page main
$t04_siswa_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t04_siswa_delete->Page_Render();
?>
...
var CurrentForm = ft04_siswadelete = new ew_Form("ft04_siswadelete", "delete");

// Form_CustomValidate event
ft04_siswadelete.Form_CustomValidate = 
...
ft04_siswadelete.ValidateRequired = true;
<?php } else { ?>
ft04_siswadelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft04_siswadelete.Lists["x_sekolah_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_Sekolah","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t02_sekolah"};
...
<?php $t04_siswa_delete->ShowPageHeader(); ?>
<?php
$t04_siswa_delete->ShowMessage();
?>
<form name="ft04_siswadelete" id="ft04_siswadelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t04_siswa_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t04_siswa_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="t04_siswa">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($t04_siswa_delete->RecKeys as $key) { ?>
...
<?php if ($t04_siswa->NIS->Visible) { // NIS ?>
		<th><span id="elh_t04_siswa_NIS" class="t04_siswa_NIS"><?php echo $t04_siswa->NIS->FldCaption() ?></span></th>
<?php } ?>
<?php if ($t04_siswa->Nama->Visible) { // Nama ?>
		<th><span id="elh_t04_siswa_Nama" class="t04_siswa_Nama"><?php echo $t04_siswa->Nama->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$t04_siswa_delete->RecCnt = 0;
$i = 0;
while (!$t04_siswa_delete->Recordset->EOF) {
	$t04_siswa_delete->RecCnt++;
	$t04_siswa_delete->RowCnt++;

	// Set row properties
	$t04_siswa->ResetAttrs();
	$t04_siswa->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$t04_siswa_delete->LoadRowValues($t04_siswa_delete->Recordset);
...
<span id="el<?php echo $t04_siswa_delete->RowCnt ?>_t04_siswa_NIS" class="t04_siswa_NIS">
...
<span id="el<?php echo $t04_siswa_delete->RowCnt ?>_t04_siswa_Nama" class="t04_siswa_Nama">
...
<?php
	$t04_siswa_delete->Recordset->MoveNext();
}
$t04_siswa_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t04_siswa_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ft04_siswadelete.Init();
</script>
<?php
$t04_siswa_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t04_siswa_delete->Page_Terminate();
?>
